...
- Github doesn't render org files very well. It's best that you clone the repository and open it in Emacs. If not possible, there's an [[https://naghdbishi.ir/Doom-Emacs-Cheat-Sheet/README.html][html version]] of README available, it's made with [[https://github.com/fniessen/org-html-themes][org-html-themes]].
...
** While coding
- =SPC c c= to run a compile comand (or a test, or any other command in the current directory)
- =SPC c C= to repeat the command above
- =SPC c d= jump to var/func/... definitions
- =SPC c D= see references to var/func/...
- =SPC c e= to evaluate the current buffer or region (when nothing is selected, equvalent to running =SPC c c= and writing =go run= + the file name.)
- =SPC c f= see references to var/func/...
- =SPC c k= jump to documentation
- =SPC c r= rename all references and defnitions for the var/func at point in all project files
